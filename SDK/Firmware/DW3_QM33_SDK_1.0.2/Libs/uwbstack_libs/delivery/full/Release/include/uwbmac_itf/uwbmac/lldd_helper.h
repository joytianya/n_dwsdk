...
struct lldd_stats {
	 ...
	 * auxiliary sub-block, FCS not included, corresponding to rx_dl_aux_frames.
	 */
...
struct lldd_helper_ops {
	 ...
	 */
	void (*state_changed)(void *user_data, enum lldd_helper_state new_state);
	/**
	 ...
	 * This callback must return quickly and it must not reenter the MAC.
	 */
	void (*network_detected)(void *user_data, const struct lldd_helper_network *network);
	/**
	 ...
	 * This callback must return quickly and it must not reenter the MAC.
	 */
	void (*association_failed)(void *user_data,
 ...
 */
enum qerr lldd_helper_open(struct lldd_context *context, struct uwbmac_context *uwbmac_context,
 ...
 * Return: QERR_SUCCESS or error.
 */
enum qerr lldd_helper_set_scheduler(struct lldd_context *context);
 ...
 * Return: 0 or error.
 */
int lldd_helper_track(struct lldd_context *context, uint16_t pan_id, uint16_t coord_short_addr);
 ...
 * Return: 0 or error.
 */
int lldd_helper_request_association(struct lldd_context *context, const uint8_t *dev_key_data,
				    size_t dev_key_size, const uint8_t *payload_data,
				    size_t payload_size);
/**
 ...
 * Return: QERR_SUCCESS or error.
 */
enum qerr lldd_helper_set_agg_limit_bytes(struct lldd_context *context, int value);
 ...
 * @value: Parameter value.
 *
 * If not 0, the device will behave as an inactive device. It will participate only in blocks with
 * an index which is a multiple of 2 to the power of inactiveExponent.
 *
 * Return: QERR_SUCCESS or error.
 */
enum qerr lldd_helper_set_inactive_exponent(struct lldd_context *context, int value);
 ...
 * Return: QERR_SUCCESS or error.
 */
enum qerr lldd_helper_set_smart_power(struct lldd_context *context, bool value);

/**
 * lldd_helper_set_tracking_loss_threshold() - Set TRACKING_LOSS_THRESHOLD parameter.
 * @context: Lldd context.
 * @value: Parameter value.
 *
 ...
 * Return: QERR_SUCCESS or error.
 */
enum qerr lldd_helper_set_tracking_loss_threshold(struct lldd_context *context, int value);
 ...
 * Return: QERR_SUCCESS or error.
 */
enum qerr lldd_helper_set_mfs_parameters(struct lldd_context *context, int mfs_id,
 ...
 * Return: QERR_SUCCESS or error.
 */
int lldd_helper_get_stats(struct lldd_context *context, struct lldd_stats *stats);
...
/**
 * lldd_helper_get_short_addr() - Get local short address assigned after
 * association.
 * @context: Lldd context.
 ...
 * Return: QERR_SUCCESS or error.
 */
enum qerr lldd_helper_get_short_addr(struct lldd_context *context, uint16_t *short_addr);
