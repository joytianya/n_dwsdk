...
#define UCI_MESSAGE_BUILDER_INITIALIZER_WITH_SIZE(uci_, expected_packet_size_)                    \
	{                                                                                         \
		.uci = (uci_), .message = NULL, .current_block = NULL, .first_errno = 0,          \
		.elems_nr = NULL, .expected_packet_size = expected_packet_size_, .alloc_flags = 0 \
	}
...
#define UCI_MESSAGE_BUILDER_INITIALIZER(uci_)                                            \
	...
	{                                                                                \
		.uci = (uci_), .message = NULL, .current_block = NULL, .first_errno = 0, \
		.elems_nr = NULL, .expected_packet_size = (uint16_t)UCI_MAX_PACKET_SIZE, \
		.alloc_flags = 0                                                         \
	}

static inline void uci_message_builder_init(struct uci_message_builder *builder, struct uci *uci)
{
...
#define UCI_MESSAGE_PARSER_INITIALIZER(message_)                                \
	{                                                                       \
		.current_block = (message_), .previous = message_, .offset = 0, \
		.current_remaining_len = (message_)->total_len,                 \
	}
 ...
 */
static inline uint8_t uci_message_nest_get_elems_nr(struct uci_message_builder *builder)
{
 ...
 */
static inline void uci_message_nest_set_elems_nr(struct uci_message_builder *builder,
						 uint8_t elems_nr)
{
	if (!builder->elems_nr)
		return;
 ...
 * not be met.
 */
enum uci_status_code uci_message_validate_builder(struct uci_message_builder *builder);
 ...
 */
int uci_message_put(struct uci_message_builder *builder, const uint8_t *data, size_t data_len);
 ...
 * Return: 0 on success, or error on allocation failure.
 */
static inline int uci_message_put_8bit(struct uci_message_builder *builder, uint8_t data)
{
 ...
 * Return: 0 on success, or error on allocation failure.
 */
static inline int uci_message_put_16bit(struct uci_message_builder *builder, uint16_t data)
{
	uint8_t buf[] = {
		(uint8_t)data,
		(uint8_t)(data >> 8),
 ...
 * Return: 0 on success, or error on allocation failure.
 */
static inline int uci_message_put_32bit(struct uci_message_builder *builder, uint32_t data)
{
	uint8_t buf[] = {
		(uint8_t)data,
		(uint8_t)(data >> 8),
 ...
 * Return: 0 on success, or error on allocation failure.
 */
static inline int uci_message_put_48bit(struct uci_message_builder *builder, uint64_t data)
{
	uint8_t buf[] = {
		(uint8_t)data,	       (uint8_t)(data >> 8),  (uint8_t)(data >> 16),
		(uint8_t)(data >> 24), (uint8_t)(data >> 32), (uint8_t)(data >> 40),
	};
 ...
 * Return: 0 on success, or error on allocation failure.
 */
static inline int uci_message_put_64bit(struct uci_message_builder *builder, uint64_t data)
{
	uint8_t buf[] = {
		(uint8_t)data,	       (uint8_t)(data >> 8),  (uint8_t)(data >> 16),
		(uint8_t)(data >> 24), (uint8_t)(data >> 32), (uint8_t)(data >> 40),
 ...
 */
int uci_message_put_nocopy(struct uci_message_builder *builder, size_t size, uint8_t **data);
 ...
 */
void uci_message_put_blk(struct uci_message_builder *builder, struct uci_blk *blk);
 ...
 */
static inline uint8_t *uci_message_reserve_8bit(struct uci_message_builder *builder, uint8_t data)
{
	uci_message_put_8bit(builder, data);
	if (!uci_message_failed(builder)) {
		return &(builder->current_block->data[builder->current_block->len - 1]);
	}
	return NULL;
}
 ...
 */
size_t uci_message_get(struct uci_message_parser *parser, uint8_t *data, size_t size);
...
{
	uint8_t data;
	__attribute__((unused)) size_t size = uci_message_get(parser, &data, sizeof(data));
...
static inline uint64_t uci_message_get_48bit(struct uci_message_parser *parser)
{
	...
	uint32_t lsb, msb;
	UCI_ASSERT(size == sizeof(data));
	lsb = data[0] | (uint32_t)data[1] << 8 | (uint32_t)data[2] << 16 | (uint32_t)data[3] << 24;
	msb = data[4] | (uint32_t)data[5] << 8 | (uint32_t)0 << 16 | (uint32_t)0 << 24;
...
static inline uint64_t uci_message_get_64bit(struct uci_message_parser *parser)
{
	...
	UCI_ASSERT(size == sizeof(data));
	lsb = data[0] | (uint32_t)data[1] << 8 | (uint32_t)data[2] << 16 | (uint32_t)data[3] << 24;
	msb = data[4] | (uint32_t)data[5] << 8 | (uint32_t)data[6] << 16 | (uint32_t)data[7] << 24;
	return lsb | (uint64_t)msb << 32;
}
 ...
 */
size_t uci_message_get_nocopy(struct uci_message_parser *parser, size_t size, uint8_t **data);
 ...
 */
enum uci_status_code uci_message_get_bytes(struct uci_message_parser *parser, uint8_t *dst,
					   uint8_t *length, size_t capacity);
 ...
 * @length: Data length.
 */
void uci_message_put_bytes(struct uci_message_builder *builder, const uint8_t *data, size_t length);
 ...
 * Return: UCI status code.
 */
enum uci_status_code uci_message_get_string(struct uci_message_parser *parser, char *dst,
					    size_t capacity);
 ...
 */
void uci_message_put_string(struct uci_message_builder *builder, const char *str, size_t maxlen);
