 ...
 * @state: State of the process.
 *
 * Return: int representing the space left in the message to encode or decode
 * values.
 */
...
/**
 * cbor_get_binary_no_cpy() - Give back a pointer to start of byte string and
 * its length.
 *
 * @state: State
 ...
 */
void cbor_get_binary_no_cpy(struct cbor_state *state, void **result, int *length);
 ...
 */
void cbor_get_binary(struct cbor_state *state, void *result, int *length, int max_length);

/**
 * cbor_get_length() - Read the length of map, array, string or byte string
 * type.
 *
 * @state: State
 *
 * When decoding, use this to read the length or array types (binary, map,
 * string, array). If reading is not possible, an error is raised.
 *
...
/**
 * cbor_put_break() - End an indefinite length map or array by adding the break
 * code.
 *
 * @state: State.
 */
