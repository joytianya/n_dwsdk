...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(dw, DWT_RESTORECONFIG, full_restore, NULL);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(dw, DWT_SETRXANTENNADELAY, 0, (void *)&antennaDly);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(dw, DWT_SETTXANTENNADELAY, 0, (void *)&antennaDly);
}
...
void dwt_setplenfine(uint16_t preambleLength)
{
    ...
    (void)dw->dwt_driver->dwt_ops->ioctl(dw, DWT_SETPLENFINE, 0, (void *)&preambleLength8Bits);
}
...
void dwt_write_rx_scratch_data(uint8_t *buffer, uint16_t length, uint16_t bufferOffset)
{
    ...
    (void)dw->dwt_driver->dwt_ops->ioctl(dw, DWT_WRITERXSCRATCHDATA, 0, &rd);
}
...
void dwt_read_rx_scratch_data(uint8_t *buffer, uint16_t length, uint16_t bufferOffset)
{
    //!!Check later if needs range protection.
    struct dwt_rw_data_s rd = { buffer, length, bufferOffset };
    (void)dw->dwt_driver->dwt_ops->ioctl(dw, DWT_READRXSCRATCHDATA, 0, &rd);
}
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(dw, DWT_WRITE_REG, address, (void *)data);
}
 ...
 */
 ...
 {
     register_name_add_t* tmp;
#if 0
//!!!! NOT working
     dw->dwt_driver->dwt_ops->ioctl(dw, DWT_DBG_REGS, 0, (void*)tmp);
#else
     tmp = (register_name_add_t*) dw->dwt_driver->dwt_ops->dbg_fn(dw, DWT_DBG_REGS, 0, NULL);
#endif
     return tmp;
 }
...
{
    struct dwt_calculate_linear_tx_setting_s tmp = {0, channel, p_indexes, p_res};
...
{
    (void)dw->dwt_driver->dwt_ops->ioctl(dw, DWT_SET_PLL_CONFIG, 0, (void *)&pll_common);
}
...
{
    struct dwt_readdiagnostics_acc_s tmp = {cir_diag, acc_idx};
